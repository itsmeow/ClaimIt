package its_meow.claimit;

public class Ref {
	
	public static final String MOD_ID = "claimit";
	public static final String NAME = "ClaimIt";
	public static final String VERSION = "1.0.0";
	public static final String acceptedMCV = "[1.12,1.13)";
	public static final String updateJSON = "http://modmanagement.net16.net/updateJSON6.json";
	public static final String CLIENT_PROXY_C = "its_meow.claimit.proxy.ClientProxy";
	public static final String SERVER_PROXY_C = "its_meow.claimit.proxy.ServerProxy";
	
}
